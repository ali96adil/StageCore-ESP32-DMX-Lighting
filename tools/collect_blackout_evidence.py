...
def inspect_log(
    ...
    *,
    ...
    required = (
        ...
        and signals["v2_blackout_only"]
        and signals["v2_remains_failsafe"]
        and len(found_ids) == 1
        and len(versions) == 1
        and output != []
    )
    status = "UNSAFE_LOG" if hazard else "SOFTWARE_LOG_CAPTURED" if required else "INCOMPLETE"
    return {
        "schema_version": 1,
        "source": source,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.out.exists():
        parser.error("refusing to overwrite existing evidence (choose a new --out)")
    try:
        ...
        result = inspect_log(raw, expected_device=args.expected_device_id,
                             source=str(args.log) if args.log else "serial:" + args.port)
    ...
    return 1 if result["software_log_status"] != "SOFTWARE_LOG_CAPTURED" else 0
